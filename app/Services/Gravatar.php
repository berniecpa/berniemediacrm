...
class Gravatar
{
    ...
    {
        if (filter_var($fallback, FILTER_VALIDATE_URL, FILTER_FLAG_PATH_REQUIRED) || in_array($fallback, array('mm', 'identicon', 'monsterid', 'wavatar', 'retro', 'blank'))) {
    ...
    {
        $url = $this->c('secure') ? $this->secureBaseUrl : $this->publicBaseUrl;
        $url .= $this->hashEmail();
