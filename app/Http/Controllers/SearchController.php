...
class SearchController extends Controller
{
    ...
    public function search()
    {
        ...
        $data['deals'] = Deal::whereLike(['title', 'contact.name', 'status', 'source.name', 'pipe.name'], $this->searchTerm)->take(30)->get();
        $data['leads'] = Lead::whereLike(['name', 'agent.name', 'status.name', 'source.name', 'job_title', 'company', 'email', 'city', 'country', 'rating_status'], $this->searchTerm)->take(30)->get();
        ...
        $data['issues'] = Issue::whereLike(['code', 'project.name', 'status.name', 'subject', 'agent.name'], $this->searchTerm)->take(30)->get();
        $data['articles'] = Knowledgebase::whereLike(['subject', 'user.name', 'category.name'], $this->searchTerm)->take(30)->get();
        $data['payments'] = Payment::whereLike(['code', 'invoice.reference_no', 'company.name', 'paymentMethod.method_name'], $this->searchTerm)->take(30)->get();
        $data['tasks'] = Task::whereLike(['name', 'project.name', 'milestone.milestone_name', 'category.name', 'user.name'], $this->searchTerm)->take(30)->get();
        $data['tickets'] = Ticket::whereLike(['code', 'subject', 'status.name', 'priority.name', 'agent.name', 'dept.deptname', 'user.name'], $this->searchTerm)->take(30)->get();
        $data['milestones'] = Milestone::whereLike(['milestone_name', 'project.name'], $this->searchTerm)->take(30)->get();
