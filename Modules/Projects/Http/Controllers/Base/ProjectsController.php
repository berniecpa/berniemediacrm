...
abstract class ProjectsController extends Controller
{
    ...
    {
        $model = $this->applyFilter()->with('company:id,name,currency', 'assignees.user');
        ...
        return DataTables::eloquent($model)
            ...
            ->editColumn('name', function ($project) {
                $str = '<i class="fas fa-' . $project->icon . ' text-' . $project->text_color . '"></i>';
                $str .= '<a class="' . themeLinks('font-semibold') . '" href="' . route('projects.view', $project->id) . '"> ' . str_limit($project->name, 30) . '</a>';
                $str .= $project->progress == 100 ? '<span class="text-success"> ✔</span>' : '';
                return $str;
            })
            ->editColumn('client_id', function ($project) {
                return $project->client_id > 0 ? '<a class="' . themeLinks('font-semibold') . '" href="' . route('clients.view', $project->client_id) . '">' . str_limit(optional($project->company)->name, 20) . '</a>' : 'N/A';
            })
            ->editColumn('billable_time', function ($project) {
                return '<span class="text-gray-600">' . secToHours($project->billable_time) . '</span>';
            })
            ->editColumn('used_budget', function ($project) {
                $color = $project->used_budget > 100 ? 'font-bold text-red-600' : 'font-bold text-green-500';
                return '<span class="' . $color . '">' . percent($project->used_budget) . '%</span>';
            })
