...
use Modules\Projects\Notifications\ProjectAssigned;
...
class ProjectsApiController extends Controller
{
    ...
    public function save(ProjectRequest $request)
    {
        ...
        foreach ($project->assignees as $member) {
            $member->user->notify(new ProjectAssigned($project));
        }
        ...
        return ajaxResponse(
            [
                ...
                'redirect' => route('projects.view', ['project' => $project->id, 'tab' => 'tasks']),
            ],
            true,
    ...
    public function tasks($id = null)
    {
        $project = $this->project->findOrFail($id);
        $this->authorize('view', $project);
        $tasks = new TasksResource($project->tasks()->with(['project:id,name', 'milestone:id,milestone_name'])->orderBy('id', 'desc')->paginate(50));
