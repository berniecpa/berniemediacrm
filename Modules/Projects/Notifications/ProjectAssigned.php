...
class ProjectAssigned extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage())
            ...
            ->greeting(langmail('projects.assigned.greeting', ['name' => $notifiable->name]))
            ->subject(langmail('projects.assigned.subject', ['project' => $this->project->name]))
            ->line(langmail('projects.assigned.body', ['name' => $this->project->name]))
            ->action(langapp('preview') . ' ' . langapp('project'), route('projects.view', ['project' => $this->project->id]));
    }
    ...
    public function toArray($notifiable)
    {
        if ($notifiable->channelActive('database', $this->type)) {
            return [
                'subject' => langmail('projects.assigned.subject', ['project' => $this->project->name]),
                'icon' => 'shield-alt',
                'activity' => langmail('projects.assigned.body', ['name' => $this->project->name]),
            ];
        }
    }
    ...
    public function toNexmo($notifiable)
    {
        if ($notifiable->channelActive('sms', $this->type)) {
            return (new NexmoMessage())
                ->content(langmail('projects.assigned.body', ['name' => $this->project->name]));
        }
    }
    ...
    public function toWhatsapp($notifiable)
    {
        if ($notifiable->channelActive('whatsapp', $this->type)) {
            return WhatsappMessage::create()
                ...
                ->message(langmail('projects.assigned.body', ['name' => $this->project->name]));
        }
    }
    /**
    ...
    public function toAwsPinpoint($notifiable)
    {
        if ($notifiable->channelActive('sms', $this->type)) {
            return (new AwsPinpointSmsMessage(
                langmail('projects.assigned.body', [
                    'name' => $this->project->name,
                ])
            ));
        }
    ...
    public function toTelegram($notifiable)
    {
        if ($notifiable->channelActive('telegram', $this->type)) {
            return TelegramMessage::create()
                ...
                ->button('View Project', route('projects.view', ['project' => $this->project->id]));
        }
    }
}
