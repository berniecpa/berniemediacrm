        ...
        <table class="table table-striped" id="tickets-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a class="text-indigo-600" href="{{ route('tickets.view', $ticket->id) }}">{{  $ticket->subject }}</a>
                    </td>
                    <td class="">{{  dateElapsed($ticket->created_at) }}</td>
                    <td>
                        <span class="label label-{{ priorityColor($ticket->priority->name) }}">@langapp(strtolower($ticket->priority->name))</span>
                    </td>
                    <td class="">{{ $ticket->dept->deptname }}</td>
                    <td>
                        <span class="label label-default">@langapp($ticket->status->name)</span>
                    </td>
