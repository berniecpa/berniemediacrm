...
class TimeEntry extends Model
{
    ...
    public function afterModelSaved()
    {
        if (!\App::runningInConsole()) {
            \Artisan::call('analytics:timesheets');
        }
    }
    ...
    {
        if ($this->task_id > 0) {
            return route('projects.view', ['project' => $this->timeable->id, 'tab' => 'tasks', 'item' => $this->task_id]);
        }
        return route('projects.view', ['project' => $this->timeable->id]);
    }
    public function setTotalAttribute($value)
    {
        $this->attributes['total'] = (!is_null($this->start) && !is_null($this->end)) ? ($this->end - $this->start) : $value;
    }
