    ...
    public function save(TimerRequest $request)
    {
        ...
        if ($entry->isTeam() || isAdmin()) {
            if ($request->has('use_dates')) {
                $request->validate(['start' => 'required|date_format:' . config('system.preferred_date_format') . ' h:i A', 'end' => 'required|date_format:' . config('system.preferred_date_format') . ' h:i A|after:start']);
                $request->request->add(['start' => dateParser($request->start, null, true)->timestamp]);
                $request->request->add(['end' => dateParser($request->end, null, true)->timestamp]);
            } else {
    ...
    public function delete($id)
    {
        ...
        return ajaxResponse(
            [
                ...
                'redirect' => route('projects.view', ['project' => $entry->timeable->id, 'tab' => 'timesheets']),
            ],
            true,
            Response::HTTP_OK
        );
