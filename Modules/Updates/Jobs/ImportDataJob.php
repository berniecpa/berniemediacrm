...
class ImportDataJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $this->data = \JsonMachine\JsonMachine::fromString($contents);
        $this->import();
        ...
        try {
            ...
            if (isset($data['fx_users'])) {
                foreach ($data['fx_users'] as $user) {
                    ...
                    $profile = $data['fx_account_details']->where('user_id', $user['id'])->first();
                ...
                foreach ($data['fx_companies'] as $client) {
                    ...
                    if (isset($data['fx_account_details'])) {
                        foreach ($data['fx_account_details']->where('company', $client['co_id']) as $contact) {
                            $usr = User::where('email', $data['fx_users']->where('id', $contact['user_id'])->first()['email'])->first();
                            $usr->profile()->update(['company' => $company->id]);
                        }
                    }
                        ...
                        foreach ($data['fx_invoices']->where('client', $client['co_id']) as $invoice) {
                            ...
                            if (isset($data['fx_payments'])) {
                                foreach ($data['fx_payments']->where('invoice', $invoice['inv_id']) as $payment) {
                                    $codeExist = \Modules\Payments\Entities\Payment::where('code', $payment['trans_id'])->count();
                                    $inv->payments()->create(
                                        [
                                            'code' => empty($payment['trans_id']) ? generateCode('payments') : ($codeExist > 0 ? $payment['trans_id'] . '-' . rand(1, 9) : $payment['trans_id']),
