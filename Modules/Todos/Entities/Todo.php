...
class Todo extends Model
{
    ...
    {
        $this->attributes['due_date'] = dbDate($value, true);
    }
    ...
    public function getTodoableUrlAttribute()
    {
        if ($this->todoable instanceof \Modules\Leads\Entities\Lead) {
            return route('leads.view', ['lead' => $this->todoable_id]);
        }
        if ($this->todoable instanceof \Modules\Tasks\Entities\Task) {
            return route('projects.view', ['project' => $this->todoable->project_id, 'tab' => 'tasks', 'item' => $this->todoable_id]);
        }
        if ($this->todoable instanceof \Modules\Expenses\Entities\Expense) {
            return route('expenses.view', ['expense' => $this->todoable_id]);
        }
        if ($this->todoable instanceof \Modules\Tickets\Entities\Ticket) {
            return route('tickets.view', ['ticket' => $this->todoable_id]);
        }
        return route('deals.view', ['deal' => $this->todoable_id]);
    }
}
