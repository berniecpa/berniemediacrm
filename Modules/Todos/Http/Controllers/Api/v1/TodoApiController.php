...
class TodoApiController extends Controller
{
    ...
    public function save(TodoRequest $request)
    {
        ...
        if (str_contains($request->subject, '[')) {
            ...
            $collection->put('due_date', \Carbon\Carbon::parse($due_date)->format(config('system.preferred_date_format') . ' h:i A'));
        }
    ...
    public function status($id = null)
    {
        ...
        $data['todo'] = $todo->id;
        $data['status'] = $todo->completed == 0 ? 'indigo-600 font-normal' : 'success line-through font-bold';
