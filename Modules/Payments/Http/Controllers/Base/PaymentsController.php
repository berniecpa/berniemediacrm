...
abstract class PaymentsController extends Controller
{
    ...
    {
        $model = $this->applyFilter()->with(['company:id,name,currency', 'invoice:id,created_at', 'paymentMethod:method_id,method_name']);
        ...
        return DataTables::eloquent($model)
            ->editColumn(
                ...
                function ($payment) {
                    ...
                    $str .= '<a class="' . themeLinks('font-semibold') . '" href="' . route('payments.view', $payment->id) . '">' . $payment->code . '</a>';
                    return $str;
                }
            )
            ->editColumn(
            ...
            ->editColumn(
                'invoice_date',
                function ($payment) {
                    return optional($payment->invoice)->id ? '<a href="' . route('invoices.view', $payment->invoice_id) . '">' . dateString($payment->invoice->created_at) . '</a>' : 'N/A';
                }
            )
            ->editColumn(
                'amount',
