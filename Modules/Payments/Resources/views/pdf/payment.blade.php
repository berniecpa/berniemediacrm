<html>
    <html>
        ...
        <title>@langapp('payment') {{ $payment->reference_no }}</title>
    @php
        ...
        <table class="width100 v-t" cellpadding="10">
            ...
            @php $data['company'] = $payment->company; @endphp
                <tr>
                    ...
                    @endif
                    <td class="width45">
                        {!! formatClientAddress($payment, 'invoice', 'billing', false) !!}
                    </td>
                    @if (get_option('swap_to_from') == 'TRUE')
                    <td class="width10">&nbsp;</td>
                    <td class="width45">
        ...
        <tbody>
            <tr class="">
                <td class="text-left width60">
                    <div class="item-name m-b-6">Payment for Invoice #{{ $payment->invoice->reference_no }}</div>
                    {!! nl2br(langapp('invoice_amount') . ': ' . formatCurrency($payment->invoice->currency, $payment->invoice->payable())) . '<br />' . langapp('balance') . ':
                    ' . formatCurrency($payment->invoice->currency, $payment->invoice->due()) !!}

                </td>
                <td class="text-center width10">{{formatQuantity(1)}}</td>
            ...
            <tr>
                ...
                <td class="text-right bg-color">{{ formatCurrency($payment->currency, $payment->amount) }}</td>
            </tr>
        </tbody>
    </table>
    <div class="m-t-40">
