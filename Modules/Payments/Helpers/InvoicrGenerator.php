...
class InvoicrGenerator implements PDFCreatorInterface
{
    public function pdf($model, $download)
    {
        ...
        $payment->addItem(
            'Payment for Invoice #' . $model->invoice->reference_no,
            nl2br(langapp('invoice_amount') . ': ' . formatCurrency($model->invoice->currency, $model->invoice->payable())) . '<br/>' . langapp('balance') . ': ' . formatCurrency($model->invoice->currency, $model->invoice->due()),
            formatQuantity(1),
            false,
            $model->amount,
            false,
