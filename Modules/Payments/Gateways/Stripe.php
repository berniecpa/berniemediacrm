...
class Stripe implements PaymentInterface
{
    ...
    public function getData($charge)
    {
        return [
            ...
            'payment_method' => AcceptPayment::firstOrCreate(['method_name' => 'Stripe'])->method_id,
            'send_email' => 1,
            'notes' => 'Paid via Stripe by ' . $this->metadata->payer,
            'meta' => [
                'payer_email' => $this->metadata->payer_email, 'txn_id' => $charge->id,
                'invoice_code' => $this->metadata->code, 'status' => $charge->status,
            ],
        ];
    }
}
