...
class Issue extends Model
{
    ...
    protected $fillable = ['code', 'project_id', 'user_id', 'assignee', 'subject', 'reproducibility', 'severity', 'priority',
        'description', 'status_id', 'closed_at', 'meta'];
    ...
    public function commentAlert($comment)
    {
        if ($this->assignee > 0) {
            ...
            $members = $this->project->assignees->filter(
                function ($member) {
    ...
    public function codeExists($next_number)
    {
        ...
        if ($this->whereCode(get_option('issue_prefix') . $next_number)->count() > 0) {
    ...
    {
        return '/projects/view/' . $this->project->id . '/issues/' . $this->id;
    }
}
