    ...
    public function toMail($notifiable)
    {
        if ($notifiable->channelActive('mail', $this->type)) {
            return (new MailMessage())
                ...
                ->action('View Issue', route('projects.view', ['project' => $this->issue->project->id, 'tab' => 'issues', 'item' => $this->issue->id]));
        }
    }
    /**
    ...
    public function toSlack($notifiable)
    {
        if ($notifiable->channelActive('slack', $this->type)) {
            return (new SlackMessage())
                ...
                ->attachment(
                    function ($attachment) {
                        $attachment->title($this->issue->subject, route('projects.view', ['project' => $this->issue->project->id, 'tab' => 'issues', 'item' => $this->issue->id]))
                            ->fields(
                                [
                                    langapp('code') => $this->issue->code,
                                    langapp('project') => $this->issue->project->name,
                                    langapp('priority') => $this->issue->priority,
                                    langapp('status') => $this->issue->status->name,
                                ]
                            );
                    }
                );
    ...
    public function toTelegram($notifiable)
    {
        if ($notifiable->channelActive('telegram', $this->type)) {
            return TelegramMessage::create()
                ...
                ->button('View Issue', route('projects.view', ['project' => $this->issue->project->id, 'tab' => 'issues', 'item' => $this->issue->id]));
        }
    }
}
