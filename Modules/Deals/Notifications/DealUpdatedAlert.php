    ...
    public function toMail($notifiable)
    {
        if ($notifiable->channelActive('mail', $this->type)) {
            return (new MailMessage())
                ...
                ->subject(langmail('deals.updated.subject', ['title' => $this->deal->title]))
    ...
    public function toNexmo($notifiable)
    {
        if ($notifiable->channelActive('sms', $this->type)) {
            return (new NexmoMessage())
                ->content(langmail('deals.updated.body', [
                    'title' => $this->deal->title,
                    'user' => $notifiable->name,
                ]));
        }
    ...
    public function toWhatsapp($notifiable)
    {
        if ($notifiable->channelActive('whatsapp', $this->type)) {
            return WhatsappMessage::create()
                ...
                ->custom($this->deal->id)
                ->message(langmail('deals.updated.body', [
                    'title' => $this->deal->title,
                    'user' => $notifiable->name,
                ]));
        }
