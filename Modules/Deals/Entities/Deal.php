...
class Deal extends Model
{
    ...
    protected $fillable = [
        ...
        'lost_time', 'lost_reason', 'source_id', 'pipeline', 'currency', 'computed_value', 'todo_percent', 'has_email', 'has_activity',
        'days_to_close', 'next_followup', 'user_id', 'archived_at',
    ];

    protected $dates = [
        'due_date', 'next_followup', 'won_time',
    ];
    protected $appends = [
    ...
    {
        return $this->company->primary_contact > 0 && !\App::runningUnitTests() ? $this->company->contact->emails()->unread()->count() : 0;
    }
    ...
    public function toProject()
    {
        ...
        $project->description = 'From ' . $this->source->name . ' in pipeline ' . $this->pipe->name;
    ...
    public function afterModelSaved()
    {
        ...
        $this->deal_value = $value;
        $this->computed_value = formatCurrency($this->currency, $value);
        $this->has_email = $this->unreadMessages() ? 1 : 0;
        $this->has_activity = $this->hasPendingActivity() ? 1 : 0;
        $this->saveQuietly();
        \Artisan::call('deals:calcstage');
    ...
    {
        return \Database\Factories\DealFactory::new();
    }
}
