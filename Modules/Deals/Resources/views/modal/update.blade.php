            ...
            <div class="tab-content">
                <div class="tab-pane fade in active" id="tab-deal-general">
                    ...
                    <div class="form-group">
                        ...
                        <select name="currency" class="form-control select2-option">
                            @foreach (currencies() as $cur)
                                <option value="{{  $cur['code']  }}" {{ get_option('default_currency') == $cur['code'] ? ' selected' : '' }}>{{ $cur['native'] }} - {{ $cur['title'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-8">
                            <select class="form-control select2-option" name="source_id" required>
                                @foreach (App\Entities\Category::select('id', 'name')->whereModule('source')->get() as $key => $source)
                                <option value="{{  $source->id  }}" {{  $source->id == $deal->source_id ? 'selected="selected"' : '' }}>{{ $source->name  }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
