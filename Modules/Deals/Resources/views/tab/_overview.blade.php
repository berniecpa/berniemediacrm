        ...
        <section class="panel panel-default">
            ...
            <section class="panel-body">
                <div class="inline m pull-right">
                    <div class="easypiechart text-success" data-percent="{{ $deal->propability }}" data-line-width="5" data-track-Color="#f0f0f0" data-bar-color="#3869D4"
                        data-rotate="180" data-scale-Color="false" data-size="80" data-animate="2000">
                ...
                <span class="thumb-sm avatar lobilist-check">
                    <img src="{{ $deal->user->profile->photo  }}" class="inline img-circle">
                ...
                <div class="m-xs">
                    ...
                    <span class="text-bold">{{  optional($deal->source)->name  }}</span>
                </div>
                <div class="m-xs">
                ...
                <div class="line"></div>
                <small class="text-xs text-uc text-muted">@langapp('project') </small>
                <div class="m-xs">
                ...
                <small class="text-xs text-uc text-muted">@langapp('company') </small>
                <div class="m-xs">
                ...
                @if (isset($deal->company->primary_contact))
                <div class="line"></div>
                <small class="text-xs text-uc text-muted">@langapp('contact_person') </small>
                <div class="m-xs">
                    <a href="{{ route('contacts.view', $deal->company->contact->id) }}">
                        <span class="">{{ $deal->company->contact->name }}</span>
                    </a>
                </div>

                <small class="text-xs text-uc text-muted">@langapp('mobile') </small>
                <div class="m-xs">
                    <span class="">{{ $deal->company->contact->profile->mobile }}</span>
                    <a href="web+aircall:{{ $deal->company->contact->profile->mobile }}" data-rel="tooltip" title="@langapp('call_via_aircall')">@icon('solid/phone','text-success')</a>
                </div>

                @endif
                <div class="map">
                    <a href="{{ $deal->company->maplink }}" rel="nofollow" target="_blank">
                        <img src="//maps.googleapis.com/maps/api/staticmap?center={{ $deal->company->map }}&amp;zoom=14&amp;scale=2&amp;size=600x340&amp;maptype=roadmap&amp;format=png&amp;visual_refresh=true&amp;key={{ config('system.google.mapkey') }}"
                            alt="Google Map">
                    </a>
                </div>
                <div class="line"></div>
                ...
                <div class="line"></div>
                <small class="text-xs text-uc text-muted">@langapp('extras') </small>
                ...
                @if($deal->status == 'lost')
                <div class="line"></div>
                <small class="text-xs text-uc text-muted">@langapp('reason') </small>
