...
abstract class DealsController extends Controller
{
    ...
    public function __construct(Deal $deal, Request $request)
    {
        ...
        $this->dealDisplayType = request('view', 'table');
    ...
    public function index()
    {
        ...
        $data['deals'] = $this->deal->whereNull('archived_at')->open()->wherePipeline($this->getPipeline())->with(['user:id,username,name'])->orderByDesc('id')->get();
    ...
    public function parseImport(CSVRequest $request, ExcelImport $importer)
    {
        ...
        return view('deals::import_fields', compact('csv_header_fields', 'csv_data', 'csv_data_file'))->with($dta);
    }
    ...
    public function open(Deal $deal)
    {
        ...
        return redirect()->route('deals.view', ['deal' => $deal->id]);
    }
    ...
    {
        $model = $this->applyFilter()->with('category:id,name', 'company:id,name,primary_contact,expense,balance,currency');

        return DataTables::eloquent($model)
            ->editColumn(
                'title',
                function ($deal) {
                    return '<a class="' . themeLinks('font-semibold') . '" href="' . route('deals.view', $deal->id) . '">' . str_limit($deal->title, 25) . '</a>';
                }
            )
            ->editColumn(
                'deal_value',
            ...
            ->editColumn(
                'assigned',
                function ($deal) {
                    return '<a class="' . themeLinks('font-semibold') . '" href="' . route('users.view', $deal->user->id) . '">' . str_limit(optional($deal->user)->name, 25) . '</a>';
                }
            )
            ->editColumn(
                'due_date',
            ...
            ->rawColumns(['title', 'deal_value', 'chk', 'assigned', 'organization'])
            ->make(true);
    }
