...
class AdminSubscriptionController extends Controller
{
    ...
    {
        $this->middleware(['auth', 'verified', '2fa', 'can:menu_subscriptions', 'can:subscriptions_all']);
            ...
            ->editColumn('action', function ($subscription) {
                ...
                if (isAdmin() || can('subscriptions_update')) {
                    if ($subscription->owner->subscription($subscription->name)->onGracePeriod()) {
                        $str .= '<a href="' . route('subscriptions.admin.activate', $subscription->id) . '" class="ml-1 ' . themeButton() . '">' . langapp('activate') . '</a>';
                    }
                    $str .= '<a href="' . route('subscriptions.admin.cancel', $subscription->id) . '" class="ml-1 btn ' . themeButton() . '" data-toggle="ajaxModal" data-rel="tooltip" title="Cancel Subscription">' . langapp('cancel') . '</a>';
                }
                return $str;
            })
    ...
    public function plansData()
    {
        ...
        return DataTables::eloquent($model)
            ...
            ->editColumn('action', function ($subscription) {
                $str = '';
                $str .= '<a href="' . route('plans.edit', $subscription->id) . '" class="ml-1 ' . themeButton() . '" data-toggle="ajaxModal"><i class="fas fa-pencil-alt"></i></a>';
                $str .= '<a href="' . route('plans.delete', $subscription->id) . '" class="ml-1 ' . themeButton() . '" data-toggle="ajaxModal"><i class="far fa-trash-alt"></i></a>';
                $str .= '<a href="' . route('plans.send', $subscription->id) . '" class="ml-1 ' . themeButton() . '" data-toggle="ajaxModal" data-rel="tooltip" title="Send"><i class="far fa-envelope-open"></i></a>';
                return $str;
            })
    ...
    public function applyFilter()
    {
        ...
        return $subscription->query()->where(function ($query) {
            $query->whereNull('ends_at')->orWhere('ends_at', '>', now())->orWhereNotNull('trial_ends_at')->where('trial_ends_at', '>', now());
        });
    }
}
