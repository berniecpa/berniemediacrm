...
class ClientSubscriptionController extends Controller
{
        ...
        return DataTables::eloquent($model)
            ...
            ->editColumn(
                ...
                function ($subscription) {
                    $str = '';
                    if (!Auth::user()->profile->business->subscribed($subscription->name)) {
                        $str .= '<a href="' . route('subscriptions.subscribe', $subscription->id) . '" target="_blank" class="ml-1 ' . themeButton() . '">' . langapp('subscribe') . '</a>';
                    } else {
                        if (Auth::user()->profile->business->subscription($subscription->name)->onGracePeriod()) {
                            $str .= '<a href="' . route('subscriptions.activate', $subscription->id) . '" class="ml-1 ' . themeButton() . '">' . langapp('activate') . '</a>';
                        }
                        $str .= '<a href="' . route('subscriptions.cancel', $subscription->id) . '" class="ml-1 ' . themeButton() . '" data-toggle="ajaxModal" data-rel="tooltip" title="Cancel Subscription"><i class="fas fa-calendar-times"></i> ' . langapp('cancel') . '</a>';
                    }
    ...
    public function applyFilter()
    {
        ...
        if ($filter === 'expired') {
            return $plans->whereDate('billing_date', '>', now());
        }
        return $plans->where('client_id', \Auth::user()->profile->company);
    }
}
