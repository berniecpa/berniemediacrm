...
class Task extends Model
{
    ...
    {
        $entry = $this->project->timesheets()->running()->whereUserId(Auth::id());
        if ($entry->count() === 0) {
            $this->project->timesheets()->create(
                [
                    'task_id' => $this->id,
                    'is_started' => 1,
                    'user_id' => Auth::id(),
    ...
    {
        return $this->isTeam() || $this->project->isClient();
    }
    ...
    {
        return formatCurrency($this->project->currency, $this->hourly_rate * $this->estimated_hours);
    }
    public function getUrlAttribute()
    {
        return '/projects/view/' . optional($this->project)->id . '/tasks/' . $this->id;
    }
    /**
