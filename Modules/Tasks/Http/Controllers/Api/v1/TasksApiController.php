...
class TasksApiController extends Controller
{
    ...
    public function index()
    {
        $tasks = new TasksResource(
            $this->task->with(['project:id,name', 'milestone:id,milestone_name'])
                ->orderByDesc('id')
                ->paginate(50)
        );
    ...
    public function copy($id)
    {
        ...
        $data = $task->replicate();
        $data = $data->toArray();
        $data['start_date'] = datePickerFormat($data['start_date']);
        $data['due_date'] = datePickerFormat($data['due_date']);
        ...
        return ajaxResponse(
            [
                ...
                'redirect' => route('projects.view', ['project' => $project->id, 'tab' => 'tasks', 'item' => $newTask->id]),
            ],
            true,
            Response::HTTP_OK
        );
    ...
    public function save(TaskRequest $request)
    {
        ...
        return ajaxResponse(
            [
                'id' => $task->id,
                'message' => langapp('saved_successfully'),
                'redirect' => route('projects.view', ['project' => $task->project_id, 'tab' => 'tasks', 'item' => $task->id]),
            ],
            true,
    ...
    {
        $task = $this->task->findOrFail($id);
        $task->update(['progress' => $this->request->done == true ? 100 : 0]);
        return ajaxResponse(
            [
                'id' => $task->id,
                'message' => langapp('changes_saved_successful'),
    ...
    public function delete($id = null)
    {
        ...
        return ajaxResponse(
            [
                ...
                'redirect' => route('projects.view', ['project' => $task->project_id, 'tab' => 'tasks']),
            ],
            true,
            Response::HTTP_OK
        );
