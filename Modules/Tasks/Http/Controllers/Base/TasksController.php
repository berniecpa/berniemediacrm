...
abstract class TasksController extends Controller
{
    ...
    public function upcoming()
    {
        ...
        return view('tasks::upcoming')->with($data);
    }
    ...
    {
        $model = $this->applyFilter($this->task->where('project_id', $project))->with('assignees.user')->orderByDesc('id');
        ...
        return DataTables::eloquent($model)
            ->editColumn('name', function ($task) {
                ...
                return $str .= '<a class="' . themeLinks() . '" href="' . route('projects.view', ['project' => $task->project_id, 'tab' => 'tasks', 'item' => $task->id]) . '"> ' . str_limit($task->name, 30) . '</a>';
            })
            ...
            ->editColumn('hours', function ($task) {
                $str = '<strong>' . secToHours($task->total_time) . '</strong>';
            ...
            ->editColumn('action', function ($task) {
                ...
                if ($task->user_id == \Auth::id() || $task->isTeam() || isAdmin()) :
                    ...
                    $str .= '" href="' . route('users.pin', ['entity' => $task->id, 'module' => 'tasks']) . '" data-rel="tooltip" title="';
