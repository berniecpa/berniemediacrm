<div class="modal-dialog">
    <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            @if ($task->project->isTeam() || can('milestones_create'))
            <div class="form-group">
                ...
                <div class="col-lg-8">
                    <select name="milestone_id" class="form-control">
                        ...
                        @foreach ($task->project->milestones as $m)
            ...
            @if ($task->project->isTeam() || can('tasks_update'))
            <div class="form-group">
            ...
            @if (isAdmin() || $task->project->isTeam())
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <div class="col-lg-8">
                    <select class="select2-option form-control" multiple="multiple" name="team[]" required>
                        @foreach ($task->project->assignees as $member)
                        <option value="{{  $member->user_id  }}" {{ $task->isTeam($member->user_id) ? 'selected' : '' }}>
                            {{  $member->user->name  }}
                        </option>
                        @endforeach
                        @if (optional($task->project->company)->primary_contact > 0)
                        <option value="{{ $task->project->company->primary_contact }}" {{ $task->isTeam($task->project->company->primary_contact) ? 'selected' : ''}}>
                            {{ $task->project->company->contact->name }}
                        </option>
                        @endif
                    </select>
                </div>
