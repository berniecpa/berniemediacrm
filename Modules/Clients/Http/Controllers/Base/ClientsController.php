...
use Modules\Clients\Jobs\BulkDeleteClients;
...
abstract class ClientsController extends Controller
{
    ...
    public function send(MailRequest $request)
    {
        ...
        $data['redirect'] = route('clients.view', $request->id);
    ...
    public function parseImport(CSVRequest $request, ExcelImport $importer)
    {
        ...
        return view('clients::import_fields', compact('csv_header_fields', 'csv_data', 'csv_data_file'))->with($dta);
    }
    ...
    public function bulkDelete()
    {
        if ($this->request->has('checked')) {
            BulkDeleteClients::dispatch($this->request->checked, \Auth::id());
    ...
    public function tableData()
    {
        ...
        return DataTables::eloquent($model)
            ->editColumn(
                'name',
                function ($client) {
                    $str = '<a class="thumb-xs avatar"> <img src="' . $client->logo . '" class="inline-block"></a>';
                    return $str . ' <a class="' . themeLinks('font-semibold') . '" href="' . route('clients.view', $client->id) . '">' . $client->name . '</a>';
                }
            )
            ->editColumn(
                'chk',
    ...
    private function applyFilter($filter)
    {
        ...
        $query->when(
            $filter == 'expenses',
            function ($qry) {
                return $qry->where('expense', '>', 0);
            }
        );
        $query->when(
            $filter == 'balance',
            function ($qry) {
                return $qry->where('balance', '>', 0);
            }
        );
        $query->when(
            $filter == 'prospects',
            function ($qry) {
                return $qry->whereDoesntHave('invoices');
            }
        );
        $query->when(
            $filter == 'customers',
            function ($qry) {
                return $qry->whereHas('invoices');
            }
        );
        return $query;
    }
