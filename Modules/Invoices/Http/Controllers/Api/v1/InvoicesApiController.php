...
class InvoicesApiController extends Controller
{
    ...
    public function save(InvoiceRequest $request)
    {
        ...
        if ($request->has('due_date')) {
            $request->request->add(['due_date' => dbDate($request->due_date)]);
        }

        $invoice = $this->invoice->create($request->except(['partial-amount', 'partial-due_date', 'partial-notes', 'id', 'tags']));
        $invoice->updateAddresses();
        if ($request->has('partial-due_date')) {
            $invoice->update(['due_date' => dbDate(request('partial-due_date.1'))]);
        }
        // TODO check for API
    ...
    public function update(InvoiceRequest $request, $id = null)
    {
        ...
        if ($request->has('tax_types')) {
            ...
            foreach ($invoice->items as $item) {
                (new TaxCalculator($invoice, $item, $request->tax_types))->saveTaxes();
            }
        }
        ...
        if ($invoice->hasPayment() == 0 && $request->has('partial-amount')) {
            $invoice->update(['due_date' => dbDate($request->input('partial-due_date.1'))]);
        }
    ...
    public function send(SendInvoiceRequest $request, $id = null)
    {
        ...
        \Mail::to($request->to)->cc($request->has('cc') ? $request->cc : [])->bcc($request->has('bcc') ? $request->bcc : [])
            ->send(new InvoiceMail($invoice, $request->subject, $request->message, $attach === true ? $invoice->pdf(false) : $attach));
        $invoice->markSent();
        return ajaxResponse(
            [
                'id' => $invoice->id,
