...
use Modules\Invoices\Jobs\MergeInvoices;
...
abstract class InvoicesController extends Controller
{
    ...
    public function parseImport(CSVRequest $request, ExcelImport $importer)
    {
        ...
        return view('invoices::import_fields', compact('csv_header_fields', 'csv_data', 'csv_data_file'))->with($dta);
    }
    ...
    public function endRecurring()
    {
        ...
        $data['redirect'] = route('invoices.view', $this->request->id);
    ...
    {
        if ($this->request->has('checked')) {
            if (count($this->request->checked) >= 2) {
                MergeInvoices::dispatch($this->request->checked)->onQueue('high');
                $data['message'] = langapp('merge_successful');
                $data['redirect'] = url()->previous();
                return ajaxResponse($data);
            }
            return response()->json(['message' => 'Merge denied', 'errors' => ['missing' => ["Please select atleast 2 invoices"]]], 500);
        }
        return response()->json(['message' => 'No invoices selected', 'errors' => ['missing' => ["Please select atleast 2 invoices"]]], 500);
    }
        ...
        return DataTables::eloquent($model)
            ->editColumn(
                ...
                function ($invoice) {
                    ...
                    $str .= '<a href="' . route('invoices.view', $invoice->id) . '" class="' . themeLinks('font-semibold') . '"> ' . $invoice->reference_no . '</a>';
    ...
    public function applyFilter()
    {
        ...
        if ($filter === 'cancelled') {
            return $this->invoice->apply(['cancelled' => 1])->whereNull('archived_at');
        }
    ...
    {
        $data['customer'] = Client::find($this->request->customer);
        return view('invoices::_includes.addresses')->with($data);
    }

    public function getTax()
    {
        return response(TaxRate::find($this->request->id));
    }
