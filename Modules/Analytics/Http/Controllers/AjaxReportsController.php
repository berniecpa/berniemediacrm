...
class AjaxReportsController extends Controller
{
    ...
    public function deals(Deal $deal)
    {
        ...
        $deals = $deal->apply($this->request->except(['range']))->with(['company', 'source', 'category', 'pipe'])->get();
    ...
    public function leads(Lead $lead)
    {
        ...
        $end_date = date('Y-m-d', strtotime($range[1]));
        $this->request->request->add(['date_range' => [$start_date, $end_date]]);
        $leads = $lead->apply($this->request->except(['range']))->with(['source', 'status'])->get();
    ...
    public function expenses(Expense $expense)
    {
        ...
        $end_date = date('Y-m-d', strtotime($range[1]));
        $this->request->request->add(['date_range' => [$start_date, $end_date]]);
        $expenses = $expense->apply($this->request->except(['range']))->with(['company:id,name', 'category:id,name'])->get();
    ...
    public function payments(Payment $payment)
    {
        ...
        $end_date = date('Y-m-d', strtotime($range[1]));
        $this->request->request->add(['date_range' => [$start_date, $end_date]]);
        $payments = $payment->apply($this->request->except(['range']))->with(['company:id,name', 'invoice:id,reference_no', 'paymentMethod'])->get();
    ...
    public function tasks(Task $task)
    {
        ...
        $end_date = date('Y-m-d', strtotime($range[1]));
        $this->request->request->add(['date_range' => [$start_date, $end_date]]);
        $tasks = $task->apply($this->request->except(['range']))->with(['project:id,name', 'milestone:id,milestone_name'])->get();
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function cards(TimeEntry $timer)
    {
        $range = explode('-', $this->request->range);
        $start_date = date('Y-m-d', strtotime($range[0]));
        $end_date = date('Y-m-d', strtotime($range[1]));
        $this->request->request->add(['date_range' => [$start_date, $end_date]]);
        $entries = $timer->apply($this->request->except(['range']))->with(['user:id,username,name'])->groupBy('user_id')->get();
        $html = view('analytics::_ajax._cards', compact(['entries', 'start_date', 'end_date']))->render();
    ...
    public function tickets(Ticket $ticket)
    {
        ...
        $end_date = date('Y-m-d', strtotime($range[1]));
        $this->request->request->add(['date_range' => [$start_date, $end_date]]);
        $tickets = $ticket->apply($this->request->except(['range']))->with(['user:id,username,name', 'dept', 'status'])->get();
