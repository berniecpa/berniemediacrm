<div class="table-responsive">
	<table id="table-payments" class="table table-striped">
		...
		<tbody>
			...
			<tr>
				<td><a href="{{ route('payments.view', $payment->id) }}">{{ $payment->code }}</a></td>
				<td><a href="{{ route('clients.view', $payment->client_id) }}">{{ optional($payment->company)->name }}</a></td>
				<td>{{ formatCurrency($payment->currency, $payment->amount) }}</td>
				<td>{{ $payment->paymentMethod->method_name }}</td>
				<td>{{ $payment->invoice->reference_no }}</td>
