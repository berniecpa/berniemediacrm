                                    ...
                                    <form>
                                        ...
                                        <div class="inline v-middle">
                                            ...
                                            <select class="form-control input-s-sm" id="filter-project" name="project">
                                                <option value="-">-</option>
                                                @foreach (Modules\Tasks\Entities\Task::select('project_id')->with('project:id,name')->where('project_id', '>', 0)->groupBy('project_id')->orderBy('id','desc')->get() as $task)
                                                <option value="{{ $task->project_id }}">{{ optional($task->project)->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="inline v-middle">
                                            ...
                                            <select class="form-control input-s-sm" id="filter-milestone" name="milestone">
                                                <option value="-">-</option>
                                                @foreach (Modules\Tasks\Entities\Task::select('id', 'milestone_id')->with('milestone:id,milestone_name')->where('milestone_id', '>', 0)->groupBy('milestone_id')->get() as $mil)
                                                <option value="{{ $mil->milestone_id }}">{{ optional($mil->milestone)->milestone_name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="inline v-middle">
