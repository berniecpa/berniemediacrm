...
class ComputeInvoices extends Command
{
    ...
    {
        $amount = 0;
        $this->invoice->whereNull('archived_at')->where('balance', '>', 0)->sent()->chunk(
            200,
            function ($invoices) use (&$amount) {
                foreach ($invoices as $invoice) {
                    if ($invoice->currency != get_option('default_currency')) {
    ...
    {
        $amount = 0;
        $cdate = now()->subMonth(1);
        $this->invoice->whereNull('cancelled_at')->whereNull('archived_at')->whereYear('created_at', $cdate->format('Y'))->whereMonth('created_at', $cdate->format('n'))->chunk(
            200,
            function ($invoices) use (&$amount) {
                foreach ($invoices as $invoice) {
                    if ($invoice->currency != get_option('default_currency')) {
