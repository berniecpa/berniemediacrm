...
class TicketsApiController extends Controller
{
    ...
    public function index()
    {
        $tickets = new TicketsResource(
            $this->ticket->with(['status', 'dept:deptid,deptname', 'priority', 'agent:id,username,name'])
                ->orderByDesc('id')
                ->paginate(50)
        );
    ...
    {
        $this->request->validate(['status_id' => 'required|numeric']);
        $ticket = $this->ticket->findOrFail($id);
        $this->authorize('update', $ticket);
        $ticket->update(['status_id' => $this->request->status_id]);
        if ($ticket->status_id == Status::where('name', 'closed')->first()->id) {
            $ticket->closeTicket();
        } else {
            event(new TicketUpdated($ticket));
        }
