...
use Modules\Tickets\Jobs\BulkOpenTickets;
...
abstract class TicketsController extends Controller
{
    ...
    public function view(Ticket $ticket)
    {
        ...
        $data['page'] = $this->getPage();
        $data['ticket'] = $ticket;
        $data['projects'] = Project::active()->select(['id', 'name'])->get();
    ...
    public function createTask(Ticket $ticket)
    {
        ...
        $data['redirect'] = route('projects.view', ['project' => $this->request->project_id, 'tab' => 'tasks', 'item' => $task->id]);
    ...
    public function bulkDelete()
    {
        if ($this->request->has('checked')) {
            BulkDeleteTickets::dispatch($this->request->checked, \Auth::id());
    ...
    {
        if ($this->request->has('checked')) {
            BulkCloseTickets::dispatch($this->request->checked, \Auth::id());
     ...
     */
    public function bulkOpen()
    {
        if ($this->request->has('checked')) {
            BulkOpenTickets::dispatch($this->request->checked, \Auth::id());
            $data['message'] = langapp('changes_saved_successful');
            $data['redirect'] = url()->previous();
            return ajaxResponse($data);
        }
    ...
    {
        $model = $this->applyFilter()->with(['user:id,username,name', 'dept:deptid,deptname', 'priority', 'status']);

        return DataTables::eloquent($model)
            ->editColumn(
                'subject',
                function ($ticket) {
                    $str = '<a class="' . themeLinks('font-semibold') . '" href="' . route('tickets.view', $ticket->id) . '">' . str_limit($ticket->subject, 30) . '</a>';
                    return $str;
                }
            )
            ->editColumn(
            ...
            ->editColumn(
                'user',
                function ($ticket) {
                    $str = is_null($ticket->closed_at) ? '<i class="far fa-comment-alt text-' . ($ticket->response_status == 'awaiting_agent' ? 'danger' : 'success') . '"></i>' : '';
                    return $str .= ' <a class="' . themeLinks('font-semibold') . '" href="' . route('users.view', $ticket->user_id) . '">' . optional($ticket->user)->name . '</a>';
                }
            )
            ->editColumn(
                'department',
            ...
            ->editColumn(
                'status',
                function ($ticket) {
                    return '<span class="text-dark">' . $ticket->statusIcon() . ' ' . langapp($ticket->status->name) . '</span>';
                }
            )
            ->editColumn(
                'priority',
                function ($ticket) {
                    return langapp(strtolower($ticket->priority->name));
                }
            )
