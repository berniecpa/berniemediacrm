                        ...
                        $attachment->title($this->ticket->subject, route('tickets.view', $this->ticket->id))
                            ->fields(
                                [
                                    ...
                                    langapp('status') => $this->ticket->status->name,
                                ]
                            );
                    }
                );
    ...
    public function toArray($notifiable)
    {
        if ($notifiable->channelActive('database', $this->type)) {
            return [
                ...
                'icon' => 'plus-circle',
                'activity' => langmail('tickets.status.body', ['subject' => $this->ticket->subject, 'status' => $this->ticket->status->name]),
            ];
        }
    }
    ...
    public function toNexmo($notifiable)
    {
        if ($notifiable->channelActive('sms', $this->type)) {
            return (new NexmoMessage())
                ->content(langmail('tickets.status.body', ['subject' => $this->ticket->subject, 'status' => $this->ticket->status->name]));
        }
    }
    ...
    public function toWhatsapp($notifiable)
    {
        if ($notifiable->channelActive('whatsapp', $this->type)) {
            return WhatsappMessage::create()
                ...
                ->message(langmail('tickets.status.body', ['subject' => $this->ticket->subject, 'status' => $this->ticket->status->name]));
        }
    }
    /**
    ...
    public function toTwilio($notifiable)
    {
        if ($notifiable->channelActive('sms', $this->type)) {
            return (new TwilioSmsMessage())
                ->content(langmail('tickets.status.body', [
                    'subject' => $this->ticket->subject,
                    'status' => $this->ticket->status->name,
                ]));
        }
