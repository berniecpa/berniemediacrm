    ...
    <section class="vbox">
        <header class="px-2 py-2 bg-white border-b border-gray-400 panel-heading">
            <div class="flex justify-between text-gray-500">
                ...
                <div>
                    @can('expenses_create')
                    <a class="btn {{themeButton()}}" data-placement="bottom" data-rel="tooltip" data-toggle="ajaxModal" href="{{route('expenses.create')}}"
                        title="@langapp('create')">
                        @icon('solid/plus') @langapp('create')
                    </a>
                    <a class="btn {{themeButton()}}" data-placement="bottom" data-rel="tooltip" data-toggle="ajaxModal" href="{{route('expenses.import')}}"
                        title="@langapp('import')">
                        @icon('solid/cloud-upload-alt') @langapp('import')
                    </a>
                    @endcan
                    @admin
                    <a href="{{route('expenses.category.show')}}" class="btn {{themeButton()}}" data-toggle="ajaxModal" data-rel="tooltip" title="@langapp('category')"
                        data-placement="bottom">
                        @icon('solid/cogs')
                    </a>
                    <a class="btn {{themeButton()}}" data-placement="bottom" data-rel="tooltip" href="{{route('expenses.export')}}" title="@langapp('download')">
            ...
            <section class="panel panel-default">
                ...
                <form id="frm-expense" method="POST">
                    ...
                    <div class="table-responsive">
                        ...
                        @can('expenses_update')
                        <button type="submit" class="btn m-1 {{themeButton()}}" value="bulk-bill">
                            <span class="">@icon('solid/check-circle') @langapp('billed')</span>
                        </button>
                        @endcan
                        @can('expenses_update')
                        <button type="submit" class="btn m-1 {{themeButton()}}" value="bulk-archive" data-rel="tooltip" title="@langapp('archive')">
                            @icon('solid/archive')
                        </button>
                        @endcan
                        @can('expenses_delete')
                        <button type="submit" class="btn m-1 {{themeButton()}}" value="bulk-delete" data-rel="tooltip" title="@langapp('delete')">
