...
class ExpensesController extends Controller
{
    ...
    public function hide(Expense $expense)
    {
        ...
        return redirect()->route('expenses.view', $expense->id);
    }
    /**
    ...
    public function parseImport(CSVRequest $request, ExcelImport $importer)
    {
        ...
        return view('expenses::import_fields', compact('csv_header_fields', 'csv_data', 'csv_data_file'))->with($dta);
    }
    ...
    {
        $model = $this->applyFilter()->with(['company:id,name,currency', 'project:id,name', 'category:id,name']);
        ...
        return DataTables::eloquent($model)
            ->editColumn(
                ...
                function ($expense) {
                    ...
                    $str .= '<a class="' . themeLinks('font-semibold') . '" href="' . route('expenses.view', $expense->id) . '"> ' . $expense->code . '</a>';
                    return $str;
                }
            )
            ->editColumn(
            ...
            ->editColumn(
                'project_id',
                function ($expense) {
                    return optional($expense->project)->id > 0 ? '<a href="' . route('projects.view', $expense->project_id) . '">' . str_limit($expense->project->name, 15) . '</a>' : 'N/A';
                }
            )
            ->editColumn(
                'client_id',
            ...
            ->editColumn(
                'category',
                function ($expense) {
                    return str_limit($expense->category->name, 15);
                }
            )
            ->editColumn(
                'expense_date',
