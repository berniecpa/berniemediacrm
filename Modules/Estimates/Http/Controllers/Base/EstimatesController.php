...
abstract class EstimatesController extends Controller
{
    ...
    public function create($client = null)
    {
        ...
        $data['deals'] = Deal::select(['id', 'title'])->open()->get();
    ...
    public function parseImport(CSVRequest $request, ExcelImport $importer)
    {
        ...
        return view('estimates::import_fields', compact('csv_header_fields', 'csv_data', 'csv_data_file'))->with($dta);
    }
    ...
    public function accepted(Estimate $estimate)
    {
        ...
        return redirect()->route('estimates.view', $estimate->id);
    }
        ...
        return DataTables::eloquent($model)
            ->editColumn(
                ...
                function ($estimate) {
                    ...
                    $str .= '<a class="' . themeLinks('font-semibold') . '" href="' . route('estimates.view', $estimate->id) . '"> ' . $estimate->reference_no . '</a>';
                    return $str;
                }
            )
            ->editColumn(
