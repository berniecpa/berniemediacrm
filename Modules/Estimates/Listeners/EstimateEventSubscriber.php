...
use Modules\Estimates\Notifications\EstimateAcceptedAlert;
use Modules\Estimates\Notifications\EstimateDeclinedAlert;
use Modules\Estimates\Notifications\EstimateViewedAlert;
...
class EstimateEventSubscriber
{
    ...
    public function onEstimateAccepted($event)
    {
        ...
        if ($event->estimate->deal_id > 0) {
            event(new DealWon($event->estimate->deal));
        }
        ...
        $user->notify(new EstimateAcceptedAlert($event->estimate));
    }
    /**
    ...
    {
        if ($event->estimate->deal_id > 0) {
            event(new DealLost($event->estimate->deal));
        }
        $event->estimate->update(
            [
                'status'          => 'Declined', 'rejected_time' => now()->toDateTimeString(),
                'rejected_reason' => request('rejected_reason'), 'accepted_time' => null
            ]
        );
        $data = [
    ...
    public function onEstimateViewed($event)
    {
        ...
        if (!is_null($event->estimate->sent_at)) {
            $user = User::role('admin')->first();
            $user->notify(new EstimateViewedAlert($event->estimate));
