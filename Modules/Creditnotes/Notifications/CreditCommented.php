...
namespace Modules\Creditnotes\Notifications;
...
class CreditCommented extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function __construct(CreditNote $creditnote)
    {
        ...
        $this->type = 'creditnote_commented';
    }
    ...
    public function toMail($notifiable)
    {
        if ($notifiable->channelActive('mail', $this->type)) {
            return (new MailMessage())
                ...
                ->subject(langmail('credits.commented.subject'))
                ->greeting(langmail('credits.commented.greeting'))
                ->line(langmail('credits.commented.body', ['name' => $this->creditnote->name]))
                ->action('View Creditnote', route('creditnotes.view', $this->creditnote->id));
        }
    }
    ...
    public function toTwilio($notifiable)
    {
        if ($notifiable->channelActive('sms', $this->type)) {
            return (new TwilioSmsMessage())
                ->content(
                    langmail('credits.commented.body', [
                        'name' => $this->creditnote->name,
                    ])
                );
        }
    ...
    public function toTelegram($notifiable)
    {
        if ($notifiable->channelActive('telegram', $this->type)) {
            return TelegramMessage::create()
                ...
                ->button('View Creditnote', route('creditnotes.view', [$this->creditnote->id]));
        }
    }
}
